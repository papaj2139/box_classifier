...
typedef struct {
    ...
    GPUBuffer *weights_winograd_gpu;
    GPUBuffer *weights_winograd_bwd_gpu;
    GPUBuffer *bias_gpu;
...
#ifdef USE_OPENCL
static void conv2d_refresh_winograd_weights(Conv2DLayerGPU *lg) {
    Conv2DLayer *l = &lg->base;
    if (!lg->weights_winograd_gpu || !lg->weights_winograd_bwd_gpu) return;
    if (l->kernel_size != 3 || l->stride != 1 || l->padding != 1) return;
    opencl_conv2d_transform_weights_winograd_3x3(lg->weights_gpu, lg->weights_winograd_gpu,
                                                 l->out_channels, l->in_channels, 0);
    opencl_conv2d_transform_weights_winograd_3x3(lg->weights_gpu, lg->weights_winograd_bwd_gpu,
                                                 l->out_channels, l->in_channels, 1);
}
#endif
...
static Tensor *conv2d_forward(Layer *self, Tensor *input, int training) {
    ...
    //run kernel
    opencl_conv2d_forward(input_gpu, lg->weights_gpu, lg->weights_winograd_gpu, lg->bias_gpu,
...
static Tensor *conv2d_backward(Layer *self, Tensor *grad_output) {
...
#ifdef USE_OPENCL
    GPUBuffer *input_gpu = tensor_get_gpu(l->input_cache);
    ...
    //zero grad_input
    opencl_zero_buffer(grad_input->gpu, grad_input->size);
    
    //run backward input kernel
    opencl_conv2d_backward_input(grad_out_gpu, lg->weights_gpu, lg->weights_winograd_bwd_gpu, grad_input->gpu,
                                 B, in_c, out_c, in_h, in_w, out_h, out_w,
                                 k, l->stride, l->padding);
    
    //weight and bias gradients on GPU
    opencl_conv2d_backward_weights(input_gpu, grad_out_gpu, lg->d_weights_gpu, lg->d_bias_gpu,
                                   B, in_c, out_c, in_h, in_w, out_h, out_w,
                                   k, l->stride, l->padding);

    tensor_gpu_invalidate_cpu(lg->base.d_weights);
    tensor_gpu_invalidate_cpu(lg->base.d_bias);
...
#ifdef USE_OPENCL
    opencl_zero_buffer(lg->d_weights_gpu, lg->base.d_weights->size);
    opencl_zero_buffer(lg->d_bias_gpu, lg->base.d_bias->size);
#endif
}
...
static void conv2d_destroy(Layer *self) {
    ...
    if (lg->weights_winograd_bwd_gpu) gpu_buffer_destroy(lg->weights_winograd_bwd_gpu);
...
Layer *layer_conv2d_create(size_t in_channels, size_t out_channels, 
    ...
    if (kernel_size == 3 && stride == 1 && padding == 1) {
        lg->weights_winograd_gpu = gpu_buffer_create((out_channels * in_channels * 16) * sizeof(float));
        lg->weights_winograd_bwd_gpu = gpu_buffer_create((out_channels * in_channels * 16) * sizeof(float));
    }
...
static Tensor *dense_backward(Layer *self, Tensor *grad_output) {
    ...
    opencl_dense_backward_weights(input_gpu, grad_out_gpu, lg->d_weights_gpu, B, in_f, out_f);
    opencl_dense_backward_bias(grad_out_gpu, lg->d_bias_gpu, B, out_f);
    tensor_gpu_invalidate_cpu(lg->base.d_weights);
    tensor_gpu_invalidate_cpu(lg->base.d_bias);
#else
...
static Tensor *flatten_forward(Layer *self, Tensor *input, int training) {
    ...
    size_t shape[] = {l->batch_size, flat_size};
    return tensor_reshape(input, 2, shape);
}

static Tensor *flatten_backward(Layer *self, Tensor *grad_output) {
    FlattenLayer *l = (FlattenLayer *)self->impl;
    return tensor_reshape(grad_output, l->original_ndim, l->original_shape);
}
...
static Tensor *sigmoid_forward(Layer *self, Tensor *input, int training) {
    ...
    l->output_cache = tensor_view(output);
    return output;
}
...
#ifdef USE_OPENCL
    GPUBuffer *grad_out_gpu = tensor_get_gpu((Tensor*)grad_output);
    GPUBuffer *sigmoid_out_gpu = tensor_get_gpu(l->output_cache);
    tensor_ensure_gpu(grad_input);

    opencl_sigmoid_backward(grad_out_gpu, sigmoid_out_gpu, grad_input->gpu, grad_output->size);
    grad_input->gpu_valid = 1;
    grad_input->cpu_valid = 0;
#else
    #pragma omp parallel for simd schedule(static)
...
static Tensor *dropout_forward(Layer *self, Tensor *input, int training) {
    DropoutLayerGPU *lg = (DropoutLayerGPU *)self->impl;
    DropoutLayer *l = &lg->base;
    
    if (!training || l->p <= 0.0f) {
        if (l->mask) {
            tensor_destroy(l->mask);
            l->mask = NULL;
        }
        return tensor_view(input);
    }
    ...
    opencl_dropout_forward(input_gpu, output->gpu, l->mask->gpu, 1.0f - l->p, l->seed++, n);
    output->gpu_valid = 1;
    output->cpu_valid = 0;
    l->mask->gpu_valid = 1;
    l->mask->cpu_valid = 0;
...
static Tensor *dropout_backward(Layer *self, Tensor *grad_output) {
    DropoutLayerGPU *lg = (DropoutLayerGPU *)self->impl;
    DropoutLayer *l = &lg->base;
    if (!l->mask) return tensor_view(grad_output);
    Tensor *grad_input = tensor_create(grad_output->ndim, grad_output->shape);
    ...
    opencl_dropout_backward(grad_out_gpu, mask_gpu, grad_input->gpu, n);
    grad_input->gpu_valid = 1;
    grad_input->cpu_valid = 0;
    return grad_input;
#endif
...
Layer *layer_dropout_create(float p) {
    Layer *layer = malloc(sizeof(Layer));
    DropoutLayerGPU *lg = calloc(1, sizeof(DropoutLayerGPU));
    DropoutLayer *l = &lg->base;
    l->p = p;
    l->seed = (unsigned int)rand();
